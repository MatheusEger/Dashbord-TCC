...
for ticker in tickers:
    ...
    try:
        ...
        if vacancia_area_valor and ocupacao_area_valor:
            # Armazenar como texto formatado
            vaga = vacancia_area_valor
            ocupa = ocupacao_area_valor

            # Para cálculo, converte temporariamente
            vaga_num = float(vaga.replace('.', '').replace(',', '.'))
            ocupa_num = float(ocupa.replace('.', '').replace(',', '.'))
            total = vaga_num + ocupa_num
            vacancia_percentual_valor = f"{(vaga_num / total) * 100:.2f}".replace('.', ',')
            ocupacao_percentual_valor = f"{(ocupa_num / total) * 100:.2f}".replace('.', ',')
            ...
            if data_ref_span:
                ...
                if match:
                    ...
                    print(f"[{ticker}] ⚠️ Data de referência não encontrada no texto.")
            else:
                data_ref = datetime.today().strftime("%Y-%m-%d")
                print(f"[{ticker}] ⚠️ Tag de data de referência não encontrada.")
